...
public class Level_3Controller extends setUp implements Initializable {
    ...
    private Monster monsterVertical;
    private Monster monsterHorizontal;
    private Timer check;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        monsterVertical = new Monster(1);
        monsterVertical.setX(20 * SIZE);
        monsterVertical.setY((4 * SIZE));
        pane.getChildren().add(monsterVertical.getCreature());
        monsterVertical.getCreature().setFitWidth(SIZE * 2);
        monsterVertical.getCreature().setFitHeight(SIZE * 2);

        monsterHorizontal = new Monster(2);
        monsterHorizontal.setX(10 * SIZE);
        monsterHorizontal.setY((28 * SIZE));
        pane.getChildren().add(monsterHorizontal.getCreature());
        monsterHorizontal.getCreature().setFitWidth(SIZE * 2);
        monsterHorizontal.getCreature().setFitHeight(SIZE * 2);


        monster.moveMonster();
        monsterVertical.moveMonsterVertically();
        monsterHorizontal.moveMonsterHorizontally();
                ...
                Platform.runLater(new Runnable() {
                    public void run() {
                        if ((monster.getCreature().getBoundsInParent().intersects(character.getPlayer().getBoundsInParent())
                                ...
                                monsterVertical.getCreature().getBoundsInParent().intersects(character.getPlayer().getBoundsInParent())
                                        && numGrid[(int) (monsterVertical.getY()) / SIZE - 2][(int) (monsterVertical.getX()) / SIZE] == 2 ||
                                monsterHorizontal.getCreature().getBoundsInParent().intersects(character.getPlayer().getBoundsInParent())
                                        && numGrid[(int) (monsterHorizontal.getY()) / SIZE - 2][(int) (monsterHorizontal.getX()) / SIZE] == 2) {
                            timer.stop();
                            if (checkLosingStatus()) {
                                monster.stopMonster();
                                monsterVertical.stopMonsterVertically();
                                monsterHorizontal.stopMonsterHorizontally();
                                stopChecking();
                            }
                            ...
                            try {
                                for (Rectangle held : rectHolder) {
                                    if (held.getBoundsInParent().intersects(monster.getCreature().getBoundsInParent()) ||
                                            held.getBoundsInParent().intersects(monsterVertical.getCreature().getBoundsInParent()) ||
                                            held.getBoundsInParent().intersects(monsterHorizontal.getCreature().getBoundsInParent())) {
                                        timer.stop();
                                        if (checkLosingStatus()) {
                                            monster.stopMonster();
                                            monsterVertical.stopMonsterVertically();
     ...
     */
    @FXML
    public void nextLevel(Event event) throws IOException {
        changeScene(event, "level_4.fxml");
    }
